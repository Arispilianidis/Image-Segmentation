function [ ] = myRecNcutsB(k,T1,img)
%MYRECNCUTSB Summary of this function goes here
...
if (~isempty(p(count-sth).WB))
    
    p(count).W=p(count-sth).WB;
     %gia memory efficient
        p(count-sth).WB=1;
    %efoson to pirame, den mas xreiazetai peretairw opote toy bazoyme tin
    %timi 1 gia na min exoyme pinaka 2500x2500(47 MB) poy den xrisimopieite
